...
class EthereumFeeModel(BlockchainModel):
    def __init__(self, alpha: float, gamma: float, max_fork: int, fee: float, transaction_chance: float,
                 max_pool: int, shard_count: int):
        self.alpha = alpha
        self.gamma = gamma
    ...
    def __repr__(self) -> str:
        return f'{self.__class__.__name__}' \
               f'({self.alpha}, {self.gamma}, {self.max_fork}, {self.fee}, {self.transaction_chance}, {self.max_pool}, {self.shard_count})'

    def __reduce__(self) -> Tuple[type, tuple]:
        return self.__class__, (
        self.alpha, self.gamma, self.max_fork, self.fee, self.transaction_chance, self.max_pool, self.shard_count)

    def get_state_space(self) -> Space:
        elements = [self.Block, self.Transaction] * (2 * self.max_fork * self.shard_count) + \
                   [self.Fork, (0, self.max_pool), (0, self.max_fork * self.shard_count),
                    (0, self.max_fork * self.shard_count), (0, self.max_pool), (0, self.max_pool)]
    ...
    def get_action_space(self) -> Space:
        return MultiDimensionalDiscreteSpace(self.Action, (0, self.max_fork * self.shard_count))

    def get_initial_state(self) -> BlockchainModel.State:
        return self.create_empty_chain() * 2 * self.shard_count + (self.Fork.Irrelevant,) + (0,) * 5

    def get_final_state(self) -> BlockchainModel.State:
        return self.create_empty_chain() * 2 * self.shard_count + (self.Fork.Irrelevant,) + (-1,) * 5

    def dissect_state(self, state: BlockchainModel.State) -> Tuple[tuple, tuple, Enum, int, int, int, int, int]:
        shard_states = [state[i:i + 2 * self.max_fork] for i in range(0, len(state) - 6, 2 * self.max_fork)]
        fork = state[-6]
        pool = state[-5]
        length_a = state[-4]
        length_h = state[-3]
        ...
        return shard_states, fork, pool, length_a, length_h, transactions_a, transactions_h

    def create_empty_chain(self) -> tuple:
        return (self.Block.NoBlock, self.Transaction.NoTransaction) * self.max_fork * self.shard_count
...
if __name__ == '__main__':
    ...
    mdp = EthereumFeeModel(0.35, max_fork=5)
